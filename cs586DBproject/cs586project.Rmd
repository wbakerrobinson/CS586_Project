...
if(connected):
    cur = conn.cursor()
    cur.execute("SELECT 1 FROM information_schema.tables WHERE table_name = 'agent';")
    ddl_run = cur.fetchone()[0]
    data_exists = []
    has_data = 0
    if(ddl_run):
      cur.execute("SELECT 1 FROM agent;")
      has_data = cur.fetchone()[0]
      data_exists.append(has_data)
      cur.execute("SELECT 1 FROM team;")
      has_data = cur.fetchone()[0]
      data_exists.append(has_data)
    else:
...
if(py$connected == TRUE){
  ...
  if(!is.null(params$availSurv) | py$ddl_run == 1)
  {
    ...
    {
      projects(params$projSurv, params$projInfo) 
      projSurvBool <- TRUE
    }
    if(py$data_exists[2] == 0 & !is.null(params$eligible))
    {
      grading(params$eligible)
      eligibleBool <- TRUE
    }
  }
}
