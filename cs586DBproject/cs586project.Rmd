...
def to_file_obj( dataframe ):
  s_buf = io.StringIO
  dataframe.to_csv(s_buf, sep = '\t', index = False, na_rep = 'NaN')
  s_buf.seek(0)
  return s_buf;
```
...
if (connected == True and r.availSurvBool == True and ddl_run == False):
  ...
  cur.copy_from(to_file_obj(r.student_df), student, null = 'NaN')
  cur.copy_from(to_file_obj(r.avail_df), availability, null = 'NaN')
  cur.copy_from(to_file_obj(r.avail_rel_df), availability_rel, null = 'NaN')
  cur.copy_from(to_file_obj(r.avail_comm_df), availability_comments, null = 'NaN')
