...
if(connected):
    cur = conn.cursor()
    table_names = ['student','availability', 'availability_rel', 'availability_comments', 'project', 
    'project_rel', 'grade_rel', 'grade', 'work_style']
    table_exists = []
    data_exists = []
    exists = 0
    has_data = 0
    for(i in 0:12)
    {
      cur.execute("SELECT 1 FROM information_schema.tables WHERE table_name = %s", (table_names[i]))
      exists = cur.fetchone()[1]
      table_exists.append(exists)
      if(exists):
      {
        cur.execute("SELECT 1 FROM %s", (table_names[i]))
        has_data = cur.fetchone()[1]
...
eligibleBool <- FALSE
projInfoBool <- FALSE
# BEFORE CLEANING CHECK IF CAN UPLOAD!!!
