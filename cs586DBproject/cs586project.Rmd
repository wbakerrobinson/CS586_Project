...
_number of students with interest level greater than 7_  
```{python question 1a}
error = False
try:
...
_number of students with interest less than or equal to 3_  
```{python question 2a}
qerror = False
try:
...
if(py$qerror == FALSE)
{
  kable(py$data, caption = "CS Topics students are most interested in") %>%
  kable_styling(bootstrap_options = c("striped"), full_width = FALSE)
}
renderVal <- UpdateProgress(renderVal, params$rendered_by_shiny)
```
...
if(py$qerror == FALSE)
{
  ...
  ggplot(data = data, mapping = aes(x = mtg_time)) +
  geom_bar() +
  labs(x = "Perferred Meeting Time", y = "Number of Students", title = "Barchart of Perferred Meeting Times") +
  theme_bw() +
  facet_wrap(~day_of_week, ncol = 2) +
  theme(axis.text.x = element_text(angle = -90))
}
