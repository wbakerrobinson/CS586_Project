...
if(py$error == FALSE)
{
  kable(py$data, col.names = c("Project Name", "Count"), caption = "Interest of greater than 5 for each project") %>%
...
_number of students with interest less than or equal to 5_  
```{python question 2a}
qerror = False
try:
  sql = "SELECT project_name, COUNT(*) FROM project_rel R JOIN project P ON (R.project_id = P.project_id) WHERE interest <= 5 GROUP BY project_name ORDER BY COUNT(*) DESC;"
...
if(py$qerror == FALSE)
{
  kable(py$data, col.names = c("Project Name", "Count"), caption = "Interest of 5 or less for each project") %>%
    kable_styling(bootstrap_options = c("striped"), full_width = FALSE)
}
renderVal <- UpdateProgress(renderVal, params$rendered_by_shiny)
```
...
qerror = False
try:
  sql = "SELECT interest, project_name FROM project_rel R JOIN project P on (R.project_id = P.project_id);"
  data = pandas.read_sql_query(sql, conn)
except:
  print("Error with query")
  qerror = True
...
if(py$qerror == FALSE)
{
  ggplot(data = py$data, mapping = aes(x = interest)) +
...
qerror = False
try:
  sql = "SELECT mtg_time, COUNT(*) AS count FROM availability_rel AR JOIN availability A on (AR.availability_id = A.availability_id) WHERE description = 'Available' GROUP BY mtg_time;"
  data = pandas.read_sql_query(sql, conn)
except:
  print("Error with query")
  qerror = True
...
if(py$qerror == FALSE)
{
    ggplot(data = py$data, mapping = aes(x = mtg_time, y = count)) +
    geom_histogram(binwidth = 1) +
    labs(x = "Available Meeting Times", y = "Number of Students", title = "Histogram of Available Meeting Times") +
    theme_bw()
}
renderVal <- UpdateProgress(renderVal, params$rendered_by_shiny)
```

### When do most students perfer to meet?
```{python question 7a}
qerror = False
try:
  sql = "SELECT mtg_time, COUNT(*) FROM availability_rel AR JOIN availability A on (AR.availability_id = A.availability_id) WHERE description = 'Perferred' GROUP BY mtg_time;"
  data = pandas.read_sql_query(sql, conn)
except:
  print("Error with query")
  qerror = True
...
if(py$qerror == FALSE)
{
    ggplot(data = py$data, mapping = aes(x = mtg_time, y = count)) +
    geom_histogram(binwidth = 1) +
    labs(x = "Perferred Meeting Time", y = "Number of Students", title = "Histogram of Perferred Meeting Times") +
    theme_bw()
}
renderVal <- UpdateProgress(renderVal, params$rendered_by_shiny)
```
