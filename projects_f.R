...
# Reading in the projects spreadsheet
  ...
  for(i in 1:nrow(projects_raw)){
    for(j in project_desc$Project_id){
      ...
      project_rel <- add_case(project_rel,
                            student_id = unlist(projects_raw[i, "student_id"]),
                            project_id = j,
  ...
  return(list(interest, skill, work_style, project_rel, role_rel, familiar_rel, interested_rel, project))
}
